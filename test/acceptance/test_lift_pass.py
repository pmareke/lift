...
class TestLiftPass:
    ...
    def test_is_holiday(self) -> None:
        response = self.client.get("/prices", query_string={"type": "1jour","date": "2019-02-25"})

        assert response.json == {'cost': 35}

    def test_is_not_holiday(self) -> None:
        response = self.client.get("/prices", query_string={"type": "1jour","date": "2024-02-26"})

        assert response.json == {'cost': 23}

    @pytest.mark.parametrize("type,cost", [
        ("1jour", 35),
        ("night", 0),
    ])
    ...
    @pytest.mark.parametrize("age,expected_cost", [
        (10, 25),
        (65, 27),
        (50, 35),
    ])
    def test_1jour_pass_with_age(self, age: int, expected_cost: int) -> None:
        response = self.client.get("/prices", query_string={"type": "1jour", "age": age})
    ...
    def test_night_pass_with_age(self, age: int, expected_cost: int) -> None:
        response = self.client.get("/prices", query_string={"type": "night", "age": age})
